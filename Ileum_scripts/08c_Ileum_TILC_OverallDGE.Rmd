---
title: "Characterization of Ileal T Cells & Innate Lymphoid Cells: Overall Differential Gene Expression (DGE) Analysis"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only T/ILC lineage lymphocytes. Let's now perform overall DGE analysis on just these clusters of cells to determine genes differentially expressed in a cluster relative to the rest of the T/ILC lineage lymphocytes in the dataset.
...
Idents(ilT) <- ilT$phyloorder # place clusters in phylo order determined by hierarchical clustering analysis
```
...
ilTDE <- FindAllMarkers(ilT, 
                       ...
                       assay = "RNA") 
ilTDE <- subset(ilTDE, p_val_adj < 0.05) # make sure the adjusted p-values are still < 0.05 since some genes in DE list have p_val_adj > 0.05
```
...
ilTDE <- ilTDE[order(ilTDE$cluster, ilTDE$p_val_adj),] # reorder by lowest to highest p-value within each cluster
```
...
write.table(ilTDE, 
            file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/TILC_OverallDE.txt") 
write_xlsx(x = ilTDE, 
           path = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/TILC_OverallDE.xlsx",
           col_names = TRUE)
```
...
ilTDE <- subset(ilTDE, avg_logFC > 0) # only take genes enriched in the clusters 
Idents(ilT) <- ilT$phyloorder
topgenes <- ilTDE %>% group_by(cluster) %>% top_n(5, avg_logFC) # only plot top 5 genes per cluster, as determined by highest average logFC values
DoHeatmap(subset(ilT, downsample = 100), # take only 100 cells per cluster for plotting
