---
title: "Characterization of Ileal Gamma Delta T Cells: Data Subsetting, Normalization, Integration, & Dimensionality Reduction"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
To define some of these cells further, we want to make the data more manageable to analyze and comprehend, and we will do so by creating subsets of our data.

For this section, we will subset and re-process cells that were identified as gd T cells.
...
ilGDT <- subset(ilT, idents = c('CD2neg GD T cells', 'Activated gd T cells', 'Cytotoxic gd T cells', 'SELLhi gd T cells', 'Cycling gd T cells')) # create a new dataset of only cells identified as gd T cells
...
```{r, warning = FALSE, error = FALSE}
counts <- as.data.frame(ilGDT[['RNA']]@counts)
...
ilGDT.list <- SplitObject(ilGDT, split.by = "SampleID") # split by sample IDs
ilGDT.list <- ilGDT.list[c("IPP1", "IPP2", "NoPP1", "NoPP2", "X2I1", "X2I2")] 
for (i in 1:length(ilGDT.list)) { # normalize data using SCTransform method
  ilGDT.list[[i]] <- SCTransform(ilGDT.list[[i]], 
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilGDT.features <- SelectIntegrationFeatures(ilGDT.list, # select the genes to use for integration
                                          verbose = TRUE) 
ilGDT.list <- PrepSCTIntegration(ilGDT.list, 
                               anchor.features = ilGDT.features,
                               verbose = TRUE)
ilGDT.anchors <- FindIntegrationAnchors(ilGDT.list, # identify anchors for integration from top 30 PCs
...
ilGDT.integrated <- IntegrateData(ilGDT.anchors, # integrate data
                                normalization.method = "SCT", 
                                dims = 1:30)
```
...
ilGDT.integrated <- RunPCA(ilGDT.integrated, # run PCA analysis for 100 dimensions of the data
                         npcs = 100, 
                         verbose = TRUE) 
```
...
```{r, warning = FALSE, error = FALSE}
pct <- ilGDT.integrated[["pca"]]@stdev / sum(ilGDT.integrated[["pca"]]@stdev) * 100 # find standard deviation for each PC
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilGDT.integrated <- RunUMAP(ilGDT.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
                          assay = "SCT") # create UMAP
ilGDT.integrated <- RunTSNE(ilGDT.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
...
```{r, warning = FALSE, error = FALSE}
DimPlot(ilGDT.integrated, 
        label = FALSE,
        reduction = 'umap',
        group.by = "cellID",
        cols = c('darkorange', 'sandybrown', 'chocolate4', 'darkorange3', 'tan')) # view UMAP plot
```
...
DimPlot(ilGDT.integrated, 
        ...
        reduction = 'tsne',
        group.by = "cellID",
        cols = c('darkorange', 'sandybrown', 'chocolate4', 'darkorange3', 'tan')) # view t-SNE plot
```
...
Identify cells of each population:
...
```{r, warning = FALSE, error = FALSE}
ilGDT.integrated <- NormalizeData(ilGDT.integrated,  
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilGDT.integrated <- ScaleData(ilGDT.integrated, 
                            assay = "RNA")
```
...
```{r, warning = FALSE, error = FALSE}
dim(ilGDT.integrated[["RNA"]]@scale.data) 
```
...
```{r, warning = FALSE, error = FALSE}
DefaultAssay(ilGDT.integrated) <- 'RNA'
saveRDS(ilGDT.integrated, file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Seurat/Ileum_GDTonly.rds") # save .rds file
```
