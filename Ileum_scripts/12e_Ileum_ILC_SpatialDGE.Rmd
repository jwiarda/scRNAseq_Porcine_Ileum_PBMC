---
title: "Characterization of Ileal ILCs: Multidimensional DGE Analysis"
author: "Jayne Wiarda"
date: "10Apr2021"
output: 
  github_document: 
...
```{r, warning = FALSE, error = FALSE}
dat.expression <- as.matrix(ilILC[['RNA']]@data) # use log-normalized data to create count matrix
```

Create a data frame of our PCs to use as our multidimensional space:

```{r, warning = FALSE, error = FALSE}
dat.pca <- as.matrix(Embeddings(object = ilILC[["pca"]]))
...
```{r, warning = FALSE, error = FALSE}
dat.tsne <- as.data.frame(Embeddings(object = ilILC[["tsne"]])) # extract t-SNE coordinates
#dat.umap <- as.data.frame(Embeddings(object = ilILC[["umap"]])) # to extract UMAP coordinates instead
```
...
```{r, warning = FALSE, error = FALSE}
FeaturePlot(ilILC, # plot expression of the top 10 DE genes on t-SNE
...
write_xlsx(res.adv$results, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/ILConly_SpatialDGE.xlsx')
```
...
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k3_GeneModules.xlsx')
det <- list()
for (i in noK){
...
df <- df[match(colnames(ilILC), rownames(df)),]
ilILC <- AddMetaData(ilILC, df)
Idents(ilILC) <- ilILC$phyloorder
genmods <- colnames(ilILC@meta.data %>% select(starts_with("GeneModule")))
VlnPlot(ilILC, 
        features = c(genmods), 
        pt.size = 0.8,
        ncol = 4)
FeaturePlot(ilILC, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            features = c(genmods),
...
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k3_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k4_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilILC, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k4_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k5_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilILC, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k5_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k6_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilILC, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k6_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k7_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilILC, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k7_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k8_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilILC, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k8_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k9_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilILC, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k9_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k10_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilILC, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/ILConly_k10_DetectionScores.xlsx')
```
