---
title: "Characterization of Ileal GD & CD8 AB T Cells: Data Subsetting, Normalization, Integration, & Dimensionality Reduction"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
For this section, we will subset and re-process cells that were identified as gd/CD8 ab T cells (clusters 3, 6, 7, 14, 17, 19, 23, 29, 31, 32, 34, 35, 37, 47, 51).
...
Create a subset of data containing only GD/CD8 T cells:
...
ilgdCD8T <- subset(ilT, idents = c('CD8 ab T cell', 'CD8/gd mix', 'gd T cell')) # create a new dataset of only cells identified as T/ILC lineage lymphocytes
...
```{r, warning = FALSE, error = FALSE}
counts <- as.data.frame(ilgdCD8T[['RNA']]@counts)
...
ilgdCD8T.list <- SplitObject(ilgdCD8T, split.by = "SampleID") # split by sample IDs
ilgdCD8T.list <- ilgdCD8T.list[c("IPP1", "IPP2", "NoPP1", "NoPP2", "X2I1", "X2I2")] 
for (i in 1:length(ilgdCD8T.list)) { # normalize data using SCTransform method
  ilgdCD8T.list[[i]] <- SCTransform(ilgdCD8T.list[[i]], 
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilgdCD8T.features <- SelectIntegrationFeatures(ilgdCD8T.list, # select the genes to use for integration
                                          verbose = TRUE) 
ilgdCD8T.list <- PrepSCTIntegration(ilgdCD8T.list, 
                               anchor.features = ilgdCD8T.features,
                               verbose = TRUE)
ilgdCD8T.anchors <- FindIntegrationAnchors(ilgdCD8T.list, # identify anchors for integration from top 30 PCs
                                      normalization.method = "SCT", 
                                      anchor.features = ilgdCD8T.features, 
                                      dims = 1:30)
ilgdCD8T.integrated <- IntegrateData(ilgdCD8T.anchors, # integrate data
                                normalization.method = "SCT", 
                                dims = 1:30)
```
...
ilgdCD8T.integrated <- RunPCA(ilgdCD8T.integrated, # run PCA analysis for 100 dimensions of the data
                         npcs = 100, 
                         verbose = TRUE) 
```
...
```{r, warning = FALSE, error = FALSE}
pct <- ilgdCD8T.integrated[["pca"]]@stdev / sum(ilgdCD8T.integrated[["pca"]]@stdev) * 100 # find standard deviation for each PC
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilgdCD8T.integrated <- RunUMAP(ilgdCD8T.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
                          assay = "SCT") # create UMAP
ilgdCD8T.integrated <- RunTSNE(ilgdCD8T.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
...
```{r, warning = FALSE, error = FALSE}
ilgdCD8T.integrated <- NormalizeData(ilgdCD8T.integrated,  
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilgdCD8T.integrated <- ScaleData(ilgdCD8T.integrated, 
                            assay = "RNA")
```
...
```{r, warning = FALSE, error = FALSE}
dim(ilgdCD8T.integrated[["RNA"]]@scale.data) 
```
...
```{r, warning = FALSE, error = FALSE}
DefaultAssay(ilgdCD8T.integrated) <- 'RNA'
saveRDS(ilgdCD8T.integrated, file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Seurat/Ileum_gdCD8Tonly.rds") # save .rds file
```
