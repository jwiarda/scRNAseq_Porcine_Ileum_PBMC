---
title: "Characterization of Ileal CD4 T Cells: Data Subsetting, Normalization, Integration, & Dimensionality Reduction"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
For this section, we will subset and re-process cells that were identified as CD4 T cells (clusters 12, 15, 26, 41, 46).
...
ilCD4T <- subset(ilT, idents = 'CD4 ab T cell') # create a new dataset of only cells identified as T/ILC lineage lymphocytes
...
```{r, warning = FALSE, error = FALSE}
counts <- as.data.frame(ilCD4T[['RNA']]@counts)
...
ilCD4T.list <- SplitObject(ilCD4T, split.by = "SampleID") # split by sample IDs
ilCD4T.list <- ilCD4T.list[c("IPP1", "IPP2", "NoPP1", "NoPP2", "X2I1", "X2I2")] 
for (i in 1:length(ilCD4T.list)) { # normalize data using SCTransform method
  ilCD4T.list[[i]] <- SCTransform(ilCD4T.list[[i]], 
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilCD4T.features <- SelectIntegrationFeatures(ilCD4T.list, # select the genes to use for integration
                                          verbose = TRUE) 
ilCD4T.list <- PrepSCTIntegration(ilCD4T.list, 
                               anchor.features = ilCD4T.features,
                               verbose = TRUE)
ilCD4T.anchors <- FindIntegrationAnchors(ilCD4T.list, # identify anchors for integration from top 30 PCs
                                      normalization.method = "SCT", 
                                      anchor.features = ilCD4T.features, 
                                      dims = 1:30,
                                      k.filter = 195) # since we have so few cells in the dataset, we have to lower our k.filter number. We tried different k.filter values in multiples of 5, and 195 is the highest number for which we could still find integration anchors.
ilCD4T.integrated <- IntegrateData(ilCD4T.anchors, # integrate data
                                normalization.method = "SCT", 
                                dims = 1:30)
```
...
ilCD4T.integrated <- RunPCA(ilCD4T.integrated, # run PCA analysis for 100 dimensions of the data
                         npcs = 100, 
                         verbose = TRUE) 
```
...
```{r, warning = FALSE, error = FALSE}
pct <- ilCD4T.integrated[["pca"]]@stdev / sum(ilCD4T.integrated[["pca"]]@stdev) * 100 # find standard deviation for each PC
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilCD4T.integrated <- RunUMAP(ilCD4T.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
                          assay = "SCT") # create UMAP
ilCD4T.integrated <- RunTSNE(ilCD4T.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
...
```{r, warning = FALSE, error = FALSE}
ilCD4T.integrated <- NormalizeData(ilCD4T.integrated,  
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilCD4T.integrated <- ScaleData(ilCD4T.integrated, 
                            assay = "RNA")
```
...
```{r, warning = FALSE, error = FALSE}
dim(ilCD4T.integrated[["RNA"]]@scale.data) 
```
...
```{r, warning = FALSE, error = FALSE}
DefaultAssay(ilCD4T.integrated) <- 'RNA'
saveRDS(ilCD4T.integrated, file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Seurat/Ileum_CD4Tonly.rds") # save .rds file
```
