---
title: "Characterization of Ileal ILCs: Overall Differential Gene Expression (DGE) Analysis"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only ILCs. Let's now perform overall DGE analysis on just these clusters of cells to determine genes differentially expressed in a cluster relative to the rest of the ILCs in the dataset.
...
Idents(ilILC) <- ilILC$phyloorder # place clusters in phylo order determined by hierarchical clustering analysis
```
...
ilILCDE <- FindAllMarkers(ilILC, 
                       ...
                       assay = "RNA") 
ilILCDE <- subset(ilILCDE, p_val_adj < 0.05) # make sure the adjusted p-values are still < 0.05 since some genes in DE list have p_val_adj > 0.05
```
...
features <- read.table("/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/QC/GeneInfo.txt", 
                       header = TRUE) # read in the gene information for the filILCered gene list from QC analysis
...
ilILCDE <- merge(ilILCDE,
              features, 
              by.x = "gene", 
              by.y = "Name") # merge the DE gene lists with the additional gene information
ilILCDE <- ilILCDE[order(ilILCDE$cluster, ilILCDE$p_val_adj),] # reorder by lowest to highest p-value within each cluster
```
...
write.table(ilILCDE, 
            file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/ILConly_OverallDE.txt") 
write_xlsx(x = ilILCDE, 
           path = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/ILConly_OverallDE.xlsx",
           col_names = TRUE)
```
...
ilILCDE <- subset(ilILCDE, avg_logFC > 0) # only take genes enriched in the clusters 
Idents(ilILC) <- ilILC$phyloorder
topgenes <- ilILCDE %>% group_by(cluster) %>% top_n(5, avg_logFC) # only plot top 5 genes per cluster, as determined by highest average logFC values
DoHeatmap(subset(ilILC, downsample = 100), # take only 100 cells per cluster for plotting
