---
title: "Characterization of Ileal Non-leukocytes: Multidimensional DGE Analysis"
author: "Jayne Wiarda"
date: "15Apr2021"
output: 
  github_document: 
...
```{r, warning = FALSE, error = FALSE}
dat.expression <- as.matrix(ilNon[['RNA']]@data) # use log-normalized data to create count matrix
```

Create a data frame of our PCs to use as our multidimensional space:

```{r, warning = FALSE, error = FALSE}
dat.pca <- as.matrix(Embeddings(object = ilNon[["pca"]]))
...
```{r, warning = FALSE, error = FALSE}
dat.tsne <- as.data.frame(Embeddings(object = ilNon[["tsne"]])) # extract t-SNE coordinates
#dat.umap <- as.data.frame(Embeddings(object = ilNon[["umap"]])) # to extract UMAP coordinates instead
```
...
```{r, warning = FALSE, error = FALSE}
FeaturePlot(ilNon, # plot expression of the top 10 DE genes on t-SNE
...
features <- read.table("/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/QC/GeneInfo.txt", 
                       header = TRUE) # read in the gene information for the filNonered gene list from QC analysis
...
write_xlsx(res.adv$results, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/NonImmune_SpatialDGE.xlsx')
```
...
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k3_GeneModules.xlsx')
det <- list()
for (i in noK){
...
df <- df[match(colnames(ilNon), rownames(df)),]
ilNon <- AddMetaData(ilNon, df)
Idents(ilNon) <- ilNon$phyloorder
genmods <- colnames(ilNon@meta.data %>% select(starts_with("GeneModule")))
VlnPlot(ilNon, 
        features = c(genmods), 
        pt.size = 0.8,
        ncol = 4)
FeaturePlot(ilNon, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            features = c(genmods),
...
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k3_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k4_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilNon, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k4_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k5_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilNon, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k5_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k6_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilNon, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k6_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k7_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilNon, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k7_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k8_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilNon, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k8_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k9_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilNon, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k9_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k10_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilNon, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/NonImmune_k10_DetectionScores.xlsx')
```
