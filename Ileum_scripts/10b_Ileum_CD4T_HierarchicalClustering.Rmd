---
title: "Characterization of Ileal CD4 T Cells: Hierarchical Clustering"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only CD4 T cells. Let's now perform hierarchical clustering analysis on just these clusters of cells.
...
DefaultAssay(ilCD4T) <- "RNA" # set our default assay to RNA for subsequent work
Idents(ilCD4T) <- ilCD4T$seurat_clusters # change our default IDs to our cluster assignments
```
...
```{r, warning = FALSE, error = FALSE}
ilCD4T <- BuildClusterTree(ilCD4T, 
                       dims = PCdims, 
                       assay = "PCA")
```
...
```{r, warning = FALSE, error = FALSE}
levels(ilCD4T) <- rev(c('46', '12', '26', '41', '15')) 
ilCD4T$phyloorder <- Idents(ilCD4T)
```
...
saveRDS(ilCD4T, "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Seurat/Ileum_CD4Tonly.rds")
```
