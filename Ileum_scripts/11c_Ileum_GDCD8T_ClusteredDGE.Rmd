---
title: "Characterization of Ileal GD & CD8 AB T Cells: Overall Differential Gene Expression (DGE) Analysis"
author: "Jayne Wiarda"
date: "15Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only GD & CD8 AB T Cells. Let's now perform overall DGE analysis on just these clusters of cells to determine genes differentially expressed in a cluster relative to the rest of the GD & CD8 AB T Cells in the dataset.
...
Idents(ilgdCD8T) <- ilgdCD8T$phyloorder # place clusters in phylo order determined by hierarchical clustering analysis
```
...
ilgdCD8TDE <- FindAllMarkers(ilgdCD8T, 
                       only.pos = FALSE, 
                       ...
                       assay = "RNA") 
ilgdCD8TDE <- subset(ilgdCD8TDE, p_val_adj < 0.05) # make sure the adjusted p-values are still < 0.05 since some genes in DE list have p_val_adj > 0.05
```
...
features <- read.table("/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/QC/GeneInfo.txt", 
                       header = TRUE) # read in the gene information for the filgdCD8Tered gene list from QC analysis
...
ilgdCD8TDE <- merge(ilgdCD8TDE,
              features, 
              by.x = "gene", 
              by.y = "Name") # merge the DE gene lists with the additional gene information
ilgdCD8TDE <- ilgdCD8TDE[order(ilgdCD8TDE$cluster, ilgdCD8TDE$p_val_adj),] # reorder by lowest to highest p-value within each cluster
```

### Save our differentially expressed gene lists

```{r, warning = FALSE, error = FALSE, eval = TRUE}
write.table(ilgdCD8TDE, 
            file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/gdCD8TOnly_OverallDE.txt") 
write_xlsx(x = ilgdCD8TDE, 
           path = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/gdCD8TOnly_OverallDE.xlsx",
           col_names = TRUE)
```

### Make a heatmap of the top DE genes

```{r, warning = FALSE, error = FALSE, fig.height = 20, fig.width = 20, message = FALSE}
ilgdCD8TDE <- subset(ilgdCD8TDE, avg_logFC > 0) # only take genes enriched in the clusters 
Idents(ilgdCD8T) <- ilgdCD8T$phyloorder
topgenes <- ilgdCD8TDE %>% group_by(cluster) %>% top_n(5, avg_logFC) # only plot top 5 genes per cluster, as determined by highest average logFC values
DoHeatmap(subset(ilgdCD8T, downsample = 100), # take only 100 cells per cluster for plotting
