---
title: "Characterization of Ileal Non-leukocytes: Hierarchical Clustering"
author: "Jayne Wiarda"
date: "10Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only non-leukocytes. Let's now perform hierarchical clustering analysis on just these clusters of cells.
...
DefaultAssay(ilNon) <- "RNA" # set our default assay to RNA for subsequent work
Idents(ilNon) <- ilNon$seurat_clusters # change our default IDs to our cluster assignments
```
...
```{r, warning = FALSE, error = FALSE}
levels(ilNon) <- rev(c('50', '45', '36')) 
ilNon$phyloorder <- Idents(ilNon)
```
...
saveRDS(ilNon, "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Seurat/Ileum_NonImmuneOnly.rds")
```
