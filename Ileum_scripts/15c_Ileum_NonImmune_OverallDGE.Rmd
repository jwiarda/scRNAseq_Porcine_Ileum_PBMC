---
title: "Characterization of Ileal Non-leukocytes: Overall Differential Gene Expression (DGE) Analysis"
author: "Jayne Wiarda"
date: "15Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only Non-leukocytes. Let's now perform overall DGE analysis on just these clusters of cells to determine genes differentially expressed in a cluster relative to the rest of the Non-leukocytes in the dataset.
...
Idents(ilNon) <- ilNon$phyloorder # place clusters in phylo order determined by hierarchical clustering analysis
```
...
ilNonDE <- FindAllMarkers(ilNon, 
                       ...
                       assay = "RNA") 
ilNonDE <- subset(ilNonDE, p_val_adj < 0.05) # make sure the adjusted p-values are still < 0.05 since some genes in DE list have p_val_adj > 0.05
```
...
ilNonDE <- merge(ilNonDE,
              features, 
              by.x = "gene", 
              by.y = "Name") # merge the DE gene lists with the additional gene information
ilNonDE <- ilNonDE[order(ilNonDE$cluster, ilNonDE$p_val_adj),] # reorder by lowest to highest p-value within each cluster
```
...
write.table(ilNonDE, 
            file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/NonImmuneOnly_OverallDE.txt") 
write_xlsx(x = ilNonDE, 
           path = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/NonImmuneOnly_OverallDE.xlsx",
           col_names = TRUE)
```
...
ilNonDE <- subset(ilNonDE, avg_logFC > 0) # only take genes enriched in the clusters 
Idents(ilNon) <- ilNon$phyloorder
topgenes <- ilNonDE %>% group_by(cluster) %>% top_n(5, avg_logFC) # only plot top 5 genes per cluster, as determined by highest average logFC values
DoHeatmap(subset(ilNon, downsample = 100), # take only 100 cells per cluster for plotting
