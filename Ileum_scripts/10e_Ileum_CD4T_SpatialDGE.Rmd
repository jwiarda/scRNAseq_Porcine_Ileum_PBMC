---
title: "Characterization of Ileal CD4 T cells: Multidimensional DGE Analysis"
author: "Jayne Wiarda"
date: "10Apr2021"
output: 
  github_document: 
...
```{r, warning = FALSE, error = FALSE}
dat.expression <- as.matrix(ilCD4T[['RNA']]@data) # use log-normalized data to create count matrix
```

Create a data frame of our PCs to use as our multidimensional space:

```{r, warning = FALSE, error = FALSE}
dat.pca <- as.matrix(Embeddings(object = ilCD4T[["pca"]]))
...
```{r, warning = FALSE, error = FALSE}
dat.tsne <- as.data.frame(Embeddings(object = ilCD4T[["tsne"]])) # extract t-SNE coordinates
#dat.umap <- as.data.frame(Embeddings(object = ilCD4T[["umap"]])) # to extract UMAP coordinates instead
```
...
```{r, warning = FALSE, error = FALSE}
FeaturePlot(ilCD4T, # plot expression of the top 10 DE genes on t-SNE
...
write_xlsx(res.adv$results, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/CD4Tonly_SpatialDGE.xlsx')
```
...
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k3_GeneModules.xlsx')
det <- list()
for (i in noK){
...
df <- df[match(colnames(ilCD4T), rownames(df)),]
ilCD4T <- AddMetaData(ilCD4T, df)
Idents(ilCD4T) <- ilCD4T$phyloorder
genmods <- colnames(ilCD4T@meta.data %>% select(starts_with("GeneModule")))
VlnPlot(ilCD4T, 
        features = c(genmods), 
        pt.size = 0.8,
        ncol = 4)
FeaturePlot(ilCD4T, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            features = c(genmods),
...
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k3_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k4_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilCD4T, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k4_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k5_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilCD4T, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k5_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k6_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilCD4T, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k6_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k7_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilCD4T, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k7_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k8_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilCD4T, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k8_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k9_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilCD4T, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k9_DetectionScores.xlsx')
```
...
res.hc.clusters <- as.data.frame(res.hc.clusters)
res.hc.clusters$gene <- rownames(res.hc.clusters)
write_xlsx(res.hc.clusters, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k10_GeneModules.xlsx')
det <- list()
for (i in noK){
  det[i] <- as.data.frame(pl[[i]][["plot_env"]][["mean.detection"]])
  }
...
FeaturePlot(ilCD4T, # I don't like the pl plot, so replotted with FeaturePlot in Seurat package
            ...
            pt.size = 2) & NoAxes()
df$CellBarcodes <- rownames(as.data.frame(pl[[1]][["plot_env"]][["mean.detection"]]))
write_xlsx(df, '/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Haystack/CD4Tonly_k10_DetectionScores.xlsx')
```
