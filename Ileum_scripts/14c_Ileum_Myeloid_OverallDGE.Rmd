---
title: "Characterization of Ileal Myeloid Lineage Leukocytes: Overall Differential Gene Expression (DGE) Analysis"
author: "Jayne Wiarda"
date: "15Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only myeloid lineage leukocytes. Let's now perform overall DGE analysis on just these clusters of cells to determine genes differentially expressed in a cluster relative to the rest of the myeloid lineage leukocytes in the dataset.
...
Idents(ilM) <- ilM$phyloorder # place clusters in phylo order determined by hierarchical clustering analysis
```
...
ilMDE <- FindAllMarkers(ilM, 
                       ...
                       assay = "RNA") 
ilMDE <- subset(ilMDE, p_val_adj < 0.05) # make sure the adjusted p-values are still < 0.05 since some genes in DE list have p_val_adj > 0.05
```
...
ilMDE <- ilMDE[order(ilMDE$cluster, ilMDE$p_val_adj),] # reorder by lowest to highest p-value within each cluster
```
...
write.table(ilMDE, 
            file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/MyeloidOnly_OverallDE.txt") 
write_xlsx(x = ilMDE, 
           path = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/MyeloidOnly_OverallDE.xlsx",
           col_names = TRUE)
```
...
ilMDE <- subset(ilMDE, avg_logFC > 0) # only take genes enriched in the clusters 
Idents(ilM) <- ilM$phyloorder
topgenes <- ilMDE %>% group_by(cluster) %>% top_n(5, avg_logFC) # only plot top 5 genes per cluster, as determined by highest average logFC values
DoHeatmap(subset(ilM, downsample = 100), # take only 100 cells per cluster for plotting
