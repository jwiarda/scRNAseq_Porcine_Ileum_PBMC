---
title: "Characterization of Ileal CD4 T Cells: Overall Differential Gene Expression (DGE) Analysis"
author: "Jayne Wiarda"
date: "10Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only CD4 T cells. Let's now perform overall DGE analysis on just these clusters of cells to determine genes differentially expressed in a cluster relative to the rest of the ILCs in the dataset.
...
Idents(ilCD4T) <- ilCD4T$phyloorder # place clusters in phylo order determined by hierarchical clustering analysis
```
...
ilCD4TDE <- FindAllMarkers(ilCD4T, 
                       only.pos = FALSE, 
                       ...
                       assay = "RNA") 
ilCD4TDE <- subset(ilCD4TDE, p_val_adj < 0.05) # make sure the adjusted p-values are still < 0.05 since some genes in DE list have p_val_adj > 0.05
```
...
features <- read.table("/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/QC/GeneInfo.txt", 
                       header = TRUE) # read in the gene information for the filCD4Tered gene list from QC analysis
...
ilCD4TDE <- merge(ilCD4TDE,
              features, 
              by.x = "gene", 
              by.y = "Name") # merge the DE gene lists with the additional gene information
ilCD4TDE <- ilCD4TDE[order(ilCD4TDE$cluster, ilCD4TDE$p_val_adj),] # reorder by lowest to highest p-value within each cluster
```

### Save our differentially expressed gene lists

```{r, warning = FALSE, error = FALSE, eval = TRUE}
write.table(ilCD4TDE, 
            file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/CD4Tonly_OverallDE.txt") 
write_xlsx(x = ilCD4TDE, 
           path = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/CD4Tonly_OverallDE.xlsx",
           col_names = TRUE)
```
...
ilCD4TDE <- subset(ilCD4TDE, avg_logFC > 0) # only take genes enriched in the clusters 
Idents(ilCD4T) <- ilCD4T$phyloorder
topgenes <- ilCD4TDE %>% group_by(cluster) %>% top_n(5, avg_logFC) # only plot top 5 genes per cluster, as determined by highest average logFC values
DoHeatmap(subset(ilCD4T, downsample = 100), # take only 100 cells per cluster for plotting
