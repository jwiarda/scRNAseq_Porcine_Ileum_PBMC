---
title: "Characterization of Ileal T Cells & Innate Lymphoid Cells: Data Subsetting, Normalization, Integration, & Dimensionality Reduction"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
Create a subset of data containing only T/ILC lineage lymphocytes:
...
ilT <- subset(il, idents = 'T/ILC lymphoid lineage leukocytes') # create a new dataset of only cells identified as T/ILC lineage lymphocytes
...
```{r, warning = FALSE, error = FALSE}
counts <- as.data.frame(ilT[['RNA']]@counts)
...
ilT.list <- SplitObject(ilT, split.by = "SampleID") # split by sample IDs
ilT.list <- ilT.list[c("IPP1", "IPP2", "NoPP1", "NoPP2", "X2I1", "X2I2")] 
for (i in 1:length(ilT.list)) { # normalize data using SCTransform method
  ilT.list[[i]] <- SCTransform(ilT.list[[i]], 
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilT.features <- SelectIntegrationFeatures(ilT.list, # select the genes to use for integration
                                          verbose = TRUE) 
ilT.list <- PrepSCTIntegration(ilT.list, 
                               anchor.features = ilT.features,
                               verbose = TRUE)
ilT.anchors <- FindIntegrationAnchors(ilT.list, # identify anchors for integration from top 30 PCs
                                      normalization.method = "SCT", 
                                      anchor.features = ilT.features, 
                                      dims = 1:30)
ilT.integrated <- IntegrateData(ilT.anchors, # integrate data
                                normalization.method = "SCT", 
                                dims = 1:30)
```
...
ilT.integrated <- RunPCA(ilT.integrated, # run PCA analysis for 100 dimensions of the data
                         npcs = 100, 
                         verbose = TRUE) 
```
...
```{r, warning = FALSE, error = FALSE}
pct <- ilT.integrated[["pca"]]@stdev / sum(ilT.integrated[["pca"]]@stdev) * 100 # find standard deviation for each PC
...
```{r, warning = FALSE, error = FALSE}
Idents(ilT.integrated) <- ilT.integrated$seurat_clusters
levels(ilT.integrated) <- c('1', '3', '6', '7', '12', '14', '15', '17', '18', '19', '23', 
                 '24', '26', '29', '31', '32', '34', '35', '37', '41', '43', '44',
                 '46', '47', '51', '53') # Reorder the clusters numerically
ilT.integrated$seurat_clusters <- Idents(ilT.integrated)
Idents(ilT.integrated) <- ilT.integrated$seurat_clusters
```

```{r, warning = FALSE, error = FALSE, message = FALSE}
ilT.integrated <- RunUMAP(ilT.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
                          assay = "SCT") # create UMAP
ilT.integrated <- RunTSNE(ilT.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
...
```{r, warning = FALSE, error = FALSE}
ilT.integrated <- NormalizeData(ilT.integrated,  
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilT.integrated <- ScaleData(ilT.integrated, 
                            assay = "RNA")
```
...
```{r, warning = FALSE, error = FALSE}
dim(ilT.integrated[["RNA"]]@scale.data) 
```
...
```{r, warning = FALSE, error = FALSE}
DefaultAssay(ilT.integrated) <- 'RNA'
saveRDS(ilT.integrated, file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Seurat/Ileum_TILConly.rds") # save .rds file
```
