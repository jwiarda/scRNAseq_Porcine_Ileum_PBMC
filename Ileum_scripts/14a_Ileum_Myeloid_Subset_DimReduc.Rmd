---
title: "Characterization of Ileal Myeloid Lineage Leukocytes: Data Subsetting, Normalization, Integration, & Dimensionality Reduction"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
Create a subset of data containing only myeloid lineage lymphocytes:
...
ilM <- subset(il, idents = 'Myeloid lineage leukocytes') # create a new dataset of only cells identified as T/ILC lineage lymphocytes
...
```{r, warning = FALSE, error = FALSE}
counts <- as.data.frame(ilM[['RNA']]@counts)
...
ilM.list <- SplitObject(ilM, split.by = "SampleID") # split by sample IDs
ilM.list <- ilM.list[c("IPP1", "IPP2", "NoPP1", "NoPP2", "X2I1", "X2I2")] 
for (i in 1:length(ilM.list)) { # normalize data using SCTransform method
  ilM.list[[i]] <- SCTransform(ilM.list[[i]], 
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilM.features <- SelectIntegrationFeatures(ilM.list, # select the genes to use for integration
                                          verbose = TRUE) 
ilM.list <- PrepSCTIntegration(ilM.list, 
                               anchor.features = ilM.features,
                               verbose = TRUE)
ilM.anchors <- FindIntegrationAnchors(ilM.list, # identify anchors for integration from top 30 PCs
...
ilM.integrated <- IntegrateData(ilM.anchors, # integrate data
                                normalization.method = "SCT", 
                                dims = 1:30)
```
...
ilM.integrated <- RunPCA(ilM.integrated, # run PCA analysis for 100 dimensions of the data
                         npcs = 100, 
                         verbose = TRUE) 
```
...
```{r, warning = FALSE, error = FALSE}
pct <- ilM.integrated[["pca"]]@stdev / sum(ilM.integrated[["pca"]]@stdev) * 100 # find standard deviation for each PC
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilM.integrated <- RunUMAP(ilM.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
                          assay = "SCT") # create UMAP
ilM.integrated <- RunTSNE(ilM.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
...
```{r, warning = FALSE, error = FALSE}
ilM.integrated <- NormalizeData(ilM.integrated,  
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilM.integrated <- ScaleData(ilM.integrated, 
                            assay = "RNA")
```
...
```{r, warning = FALSE, error = FALSE}
dim(ilM.integrated[["RNA"]]@scale.data) 
```
...
```{r, warning = FALSE, error = FALSE}
DefaultAssay(ilM.integrated) <- 'RNA'
saveRDS(ilM.integrated, file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Seurat/Ileum_MyeloidOnly.rds") # save .rds file
```
