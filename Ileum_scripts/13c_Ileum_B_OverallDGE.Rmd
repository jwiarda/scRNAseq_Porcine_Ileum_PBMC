---
title: "Characterization of Ileal B Lineage Lymphocytes: Overall Differential Gene Expression (DGE) Analysis"
author: "Jayne Wiarda"
date: "11Apr2021"
output: 
  github_document: 
...
We have now created a data subset consisting of only B lineage lymphocytes. Let's now perform overall DGE analysis on just these clusters of cells to determine genes differentially expressed in a cluster relative to the rest of the B lineage lymphocytes in the dataset.
...
Idents(ilB) <- ilB$phyloorder # place clusters in phylo order determined by hierarchical clustering analysis
```
...
ilBDE <- FindAllMarkers(ilB, 
                       ...
                       assay = "RNA") 
ilBDE <- subset(ilBDE, p_val_adj < 0.05) # make sure the adjusted p-values are still < 0.05 since some genes in DE list have p_val_adj > 0.05
```
...
ilBDE <- ilBDE[order(ilBDE$cluster, ilBDE$p_val_adj),] # reorder by lowest to highest p-value within each cluster
```
...
write.table(ilBDE, 
            file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/B_OverallDE.txt") 
write_xlsx(x = ilBDE, 
           path = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DE/B_OverallDE.xlsx",
           col_names = TRUE)
```
...
ilBDE <- subset(ilBDE, avg_logFC > 0) # only take genes enriched in the clusters 
Idents(ilB) <- ilB$phyloorder
topgenes <- ilBDE %>% group_by(cluster) %>% top_n(5, avg_logFC) # only plot top 5 genes per cluster, as determined by highest average logFC values
DoHeatmap(subset(ilB, downsample = 100), # take only 100 cells per cluster for plotting
