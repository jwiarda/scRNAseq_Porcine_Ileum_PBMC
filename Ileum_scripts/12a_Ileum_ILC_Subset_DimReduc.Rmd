---
title: "Characterization of Ileal ILCs: Data Subsetting, Normalization, Integration, & Dimensionality Reduction"
author: "Jayne Wiarda"
date: "9Apr2021"
output: 
  github_document: 
...
For this section, we will subset and re-process cells that were identified as ILCs (clusters 1, 18, 43, 44, 53).
...
Create a subset of data containing only ILCs:
...
ilILC <- subset(ilT, idents = c('ILC')) # create a new dataset of only cells identified as ILCs
...
```{r, warning = FALSE, error = FALSE}
counts <- as.data.frame(ilILC[['RNA']]@counts)
...
ilILC.list <- SplitObject(ilILC, split.by = "SampleID") # split by sample IDs
ilILC.list <- ilILC.list[c("IPP1", "IPP2", "NoPP1", "NoPP2", "X2I1", "X2I2")] 
for (i in 1:length(ilILC.list)) { # normalize data using SCTransform method
  ilILC.list[[i]] <- SCTransform(ilILC.list[[i]], 
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilILC.features <- SelectIntegrationFeatures(ilILC.list, # select the genes to use for integration
                                          verbose = TRUE) 
ilILC.list <- PrepSCTIntegration(ilILC.list, 
                               anchor.features = ilILC.features,
                               verbose = TRUE)
ilILC.anchors <- FindIntegrationAnchors(ilILC.list, # identify anchors for integration from top 30 PCs
                                      normalization.method = "SCT", 
                                      anchor.features = ilILC.features, 
                                      dims = 1:30)
ilILC.integrated <- IntegrateData(ilILC.anchors, # integrate data
                                normalization.method = "SCT", 
                                dims = 1:30)
```
...
ilILC.integrated <- RunPCA(ilILC.integrated, # run PCA analysis for 100 dimensions of the data
                         npcs = 100, 
                         verbose = TRUE) 
```
...
```{r, warning = FALSE, error = FALSE}
pct <- ilILC.integrated[["pca"]]@stdev / sum(ilILC.integrated[["pca"]]@stdev) * 100 # find standard deviation for each PC
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilILC.integrated <- RunUMAP(ilILC.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
                          assay = "SCT") # create UMAP
ilILC.integrated <- RunTSNE(ilILC.integrated, 
                          dims = PCdims, 
                          reduction = "pca", 
...
```{r, warning = FALSE, error = FALSE}
ilILC.integrated <- NormalizeData(ilILC.integrated,  
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
ilILC.integrated <- ScaleData(ilILC.integrated, 
                            assay = "RNA")
```
...
```{r, warning = FALSE, error = FALSE}
dim(ilILC.integrated[["RNA"]]@scale.data) 
```
...
```{r, warning = FALSE, error = FALSE}
DefaultAssay(ilILC.integrated) <- 'RNA'
saveRDS(ilILC.integrated, file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/Seurat/Ileum_ILConly.rds") # save .rds file
```
