...
All <- readRDS("/home/Jayne.Wiarda/scRNAseqIleumAtlas/PBMC/Scrublet/PBMCScrubbedSeurat.rds") # read in Seurat object from doublet removal step
All.list <- SplitObject(All, split.by = "SampleID") # split by sample IDs
All.list <- All.list[c("PBMC1", "PBMC2")] # specify order
```
...
```{r, warning = FALSE, error = FALSE, fig.width = 30, fig.height = 25}
DefaultAssay(All.integrated) <- "SCT"
FeaturePlot(All.integrated,
            features = c("CD3G", "CD3E", "CD79B", "JCHAIN", "CST3", 
            "CD4", "CD8B", "TRDC", "PCLAF", "CCL5", "AICDA",
...
The clustering resolution we liked best was 2, and we assign this to the meta data term 'seurat_clusters'. We continue to store the other cluster assignments at other resolutions too, just in case someone wants to perform analysis at a different resolution. We chose resolution = 2 as we thought it balanced between minimizing the number of clusters recovered and maximizing segregation of cells based on previously known phenotype/functional biology (canonical gene expression patterns).

```{r, warning = FALSE, error = FALSE}
All.integrated[["seurat_clusters"]] <- All.integrated[["integrated_snn_res.2"]] 
```
...
saveRDS(All.integrated, file = "/home/Jayne.Wiarda/scRNAseqIleumAtlas/PBMC/Seurat/PBMC.rds") 
```
