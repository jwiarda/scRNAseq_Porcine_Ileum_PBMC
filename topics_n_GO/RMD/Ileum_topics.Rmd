...
* See last tab for the functions used to generate the results
```{r, eval=FALSE}

# helper tibble to line things up 
work <- 
  tibble(
  group_names=sub('.rds','',list.files('./data/', pattern = '.rds')), 
  SEURATS=lapply(list.files('./data/', pattern = '.rds', full.names = T), readRDS), 
  FITS=lapply(list.files('./topic_model_fits/', pattern = '.rds', full.names = T), readRDS)) 

# call to the analyze_fits function
# calls other functions within
map2(.x=work$SEURATS, .y=work$FITS, ..3=work$group_names,
     .f=~analyze_fits(SEURAT = .x, FITS = .y,GROUP = ..3, SEED=2)))

# writes out results objects for each subset of cells
